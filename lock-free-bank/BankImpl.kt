...
class BankImpl(override val numberOfAccounts: Int) : Bank {
    ...
    override fun withdraw(index: Int, amount: Long): Long {
        require(amount > 0) { "Invalid amount: $amount" }
        check(amount <= MAX_AMOUNT) { "Underflow" }

        while (true) {
            val account = account(index)
            if (account.invokeOperation()) continue  // complex operation in progress, help
    ...
    private fun acquire(index: Int, op: Op): AcquiredAccount? {
         ...
         */
        while (true) {
            val account = account(index)  // volatile read creates (op.invokeOperation() happens-before op.completed == true)
            if (op.completed) return null
            if (account is AcquiredAccount) {
                if (account.op == op) return account  // account already acquired for this task
                account.invokeOperation()  // help complete other task
            } else {
    ...
    private inner class TransferOp(val fromIndex: Int, val toIndex: Int, val amount: Long) : Op() {
        ...
        override fun invokeOperation() {
            ...
            if (from != null && to != null) {  // task is not completed yet
                when {
